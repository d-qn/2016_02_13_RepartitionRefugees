...
# Define the data!
data <- c(328 + 301)
data <- c(data, 6202 - data)
data <- c(data, 98256 - sum(data))
data <- c(data,  7744 + 54000)
...
waffleduc <- function(parts, rows = 10, xlab=NULL, title=NULL, colors=NA,
                   ...
                   plot.title = element_text(size=24,  family = "Open Sans Semibold", hjust = 0, vjust = -0.1)) {
  ...
  if (inherits(use_glyph, "logical")) {
    ...
    gg <- gg + scale_fill_manual(name="",
                                 values=colors,
                                 labels=part_names)
    gg <- gg + guides(fill=guide_legend(nrow = 2, byrow = TRUE, override.aes=list(colour=NULL)))
...
for (lang in colnames(txt)) {
  ...
  dd <- data
  title <- paste(strwrap(txt['main.title', lang], width = 48, prefix = "\n ", initial = ""), collapse ="")
  ...
  wchart <- waffleduc(dd, rows = 200, size = 1.2, color = c('#333366', '#ab3d3f', '#aa8959', "darkgrey"), 
    ...
    title = paste0("\n", txt['descr', lang], "\n", txt['descr2', lang], "\n", txt['descr3', lang], "\n"),
    plot.title = element_text(size = title.fsize,  family = txt['font', lang], hjust = 0, vjust = -0.1),
